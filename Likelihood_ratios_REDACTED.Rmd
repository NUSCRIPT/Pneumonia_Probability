...
row1 <- subset(vap_npc_df, patient_ir_id == redacted)
bal_results$bal_collection_date <- as.character(bal_results$bal_collection_date)
row1_bal <- subset(bal_results, patient_ir_id == redacted & bal_collection_date == "redacted")
row1_merged <- merge(row1, row1_bal, by.x=c("patient_ir_id"), by.y=c("patient_ir_id"))

row2 <- subset(vap_npc_df, patient_ir_id == redacted)
row2_bal <- subset(bal_results, patient_ir_id == redacted & bal_collection_date == "redacted")
row2_merged <- merge(row2, row2_bal, by.x=c("patient_ir_id"), by.y=c("patient_ir_id"))

row3 <- subset(vap_npc_df, patient_ir_id ==redacted)
row3_bal <- subset(bal_results, patient_ir_id == redacted & bal_collection_date == "redacted")
